...
// * <p>
// * ***整个权限的申请与处理的过程是这样的： *****1.进入主Activity，首先申请所有的权限； *****2.用户对权限进行授权，有2种情况：
// * ********1).用户Allow了权限，则表示该权限已经被授权，无须其它操作；
// * ********2).用户Deny了权限，则下次启动Activity会再次弹出系统的Permisssions申请授权对话框。
// * *****3.如果用户Deny了权限，那么下次再次进入Activity，会再次申请权限，这次的权限对话框上，会有一个选项“dont ask me
// * again”： ********1).如果用户勾选了“dont ask me
// * again”的checkbox，下次启动时就必须自己写Dialog或者Snackbar引导用户到应用设置里面去手动授予权限；
...
//     * ***4).设备的系统设置中
//     * ，禁止了应用获取这个权限的授权，则调用shouldShowRequestPermissionRationale()，返回false。
//     */
